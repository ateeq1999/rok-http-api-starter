...
pub async fn list_roles(State(state): State<AppState>) -> Result<ApiResponse, AppError> {
...
pub async fn create_role(
  ...
  let role =
    auth::services::rbac_service::create_role(&state, &input.name, input.description.as_deref())
      .await?;
...
pub async fn grant_permission(
  ...
  auth::services::rbac_service::grant_permission_to_role(&state, &role_id, &input.permission_id)
    .await?;
  Ok(ApiResponse::ok(
    serde_json::json!({ "message": "permission granted" }),
  ))
}
...
pub async fn revoke_permission(
  ...
  auth::services::rbac_service::revoke_permission_from_role(&state, &role_id, &permission_id)
    .await?;
  Ok(ApiResponse::ok(
    serde_json::json!({ "message": "permission revoked" }),
  ))
}

pub async fn list_permissions(State(state): State<AppState>) -> Result<ApiResponse, AppError> {
...
pub async fn assign_role(
  ...
  Ok(ApiResponse::ok(
    serde_json::json!({ "message": "role assigned" }),
  ))
}
...
pub async fn my_permissions(user: auth::extractors::AuthUser) -> Result<ApiResponse, AppError> {
  let permissions: Vec<&str> = user
    .claims
    .permissions
    .split(',')
    .filter(|s| !s.is_empty())
